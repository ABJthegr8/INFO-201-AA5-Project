...
my.server <- function(input, output) {
  output$country.graph <- renderPlotly( {
    ...
    ##
    # trial <- filter(countries.filtered.48.edu.employ.good.data, Country.Code == "KOR", Series.Code == "SL.UEM.1524.FM.ZS")
    # T.males <- filter(countries.filtered.48.edu.employ.good.data, Country.Code == "KOR", Series.Code == gsub(".FE", ".MA", trial$Series.Code))
    # tmales.mani <- select(T.males, -Series, -Series.Code, -Country.Name, -Country.Code, -NA_count)
    # f.mani <- select(trial, -Series, -Series.Code, -Country.Name, -Country.Code, -NA_count)
    # t.together <- rbind(years, tmales.mani, f.mani)
    # right.before <- data.frame(t(t.together))
    # finalized <- na.omit(right.before)
    # finalized$X3 <- 100 - finalized$X2    
